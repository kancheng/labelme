"""
HTML 報表（與 segmentation-and-detection 相同）：依 YOLO 訓練/預測結果產生 index.html。
複製自 segmentation-and-detection/function/fdash.py，改為不依賴 imutils（用 cv2.resize）。
"""
import os
...
def _resize_by_height(img, height):
    """依指定高度等比例縮放（取代 imutils.resize）。"""
    h, w = img.shape[:2]
    if h <= 0:
        return img
...
def report_function_d(ymal_path, original_image, predict_folder, train_folder, html_path, pout):
    ...
    for filename in os.listdir(train_folder):
        if filename.lower().endswith(allowed_img_exts):
            ...
            h, w, _ = img.shape
            img_resize = _resize_by_height(img, h // 2)
...
# <h4 id='t1w1i2'>Image 2</h4><p><img src='./cow.jpg' alt='cow.jpg'></p>
    ...
    for stem in equal_lists:
        ...
        h, w, _ = img.shape
        img_resize = _resize_by_height(img, h // 4)
        h, w, _ = img_resize.shape
        print("H : ", h ,"; W : ", w)
        image = cv2.imencode('.png', img_resize)[1]
        base64_head = 'data:image/png;base64,'
